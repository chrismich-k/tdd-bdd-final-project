...
class TestProductRoutes(TestCase):
    ...
    def test_create_product(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        new_product = response.get_json()[0]
    ...
    def test_get_product(self):
        """It should get a product from the API"""
    ...
    def test_get_product_not_found(self):
        """It should attempt to get a product that doesn't exist"""
        invalid_id = 0
    ...
    def test_update_product(self):
        """It should update an existing product and validate they changes made"""
        # create a new product object and POST it to the API
        test_product = ProductFactory()
        response = self.client.post(BASE_URL, json=test_product.serialize())
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        new_product_data["description"] = "Short Test Description."
        update_response = self.client.put(f"{BASE_URL}/{new_product_data['id']}", json=new_product_data)
        self.assertEqual(update_response.status_code, status.HTTP_200_OK)

        # check if returned data matches the data sent by PUT
        returned_data = update_response.get_json()
        self.assertEqual(returned_data["description"], new_product_data["description"])
    ...
    def test_delete_product(self):
        ...
        delete_response = self.client.delete(f"{BASE_URL}/{products[0].id}")
        self.assertEqual(delete_response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(delete_response.data), 0)

        # try to access the deleted prodauct
        get_response = self.client.get(f"{BASE_URL}/{products[0].id}")
        self.assertEqual(get_response.status_code, status.HTTP_404_NOT_FOUND)

        # count the remaining products in the DB, check it has decreased
